...
@WebServlet("/UserFindServlet") // 이렇듯 WebServlet annotation 에서 정의를 해주면 굳이 web.xml에서 정의를 해줄필요 없이 서블릿으로 정상적으로 인식하게 된다.
public class UserFindServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		} else if(new UserDAO().registerCheck(userID) == 0) { // 0: 이미 존재하는 회원 
			try {
